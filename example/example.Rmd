...
This document aims at showing the use of some of the functions contained in the `ilaprosUtils` package. I will use the data obtained via the package `rnrfa`, which retrives data from the National River Flow Archive. Indeed, `ilaprosUtils` is mostly a combination of functions I developed (and continue to develop) in my years at the Centre for Ecology and Hydrology and can be useful when working with hydrological data, in particular high end extremes. 
...
The water year in the UK is commonly taken to start in October. Sumemr is taken to be th eperiod running between March and October. We identify the Water year with the `date2wy` function and the summer month with the `date2summer` function. 
...
Years in which less than 80% of the flow data are recorded are to be considered incomplete.  If the interest lies in some extremal part of the distribution, one should be careful when keeping records with missing data in the datast: it could be that we do not record the part of the flow which are actually of interest to the analysis. 
```{r, cache=TRUE}
...
To extract the annual maxima, we use the `inst2daily` function, which needs as input a matrix fo data, and the information of which column stores the variable of intereset (`indVec`) and which column stores the (factor) variable whcih should be used to divide the data (`indFac`):
